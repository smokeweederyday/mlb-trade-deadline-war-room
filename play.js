...
async function loadPlay() {
  ...
  try {
    ...
    setText("awayTeam", play.away_team);
    setText("homeTeam", play.home_team);
    setText("playSport", `${play.sport} // ${play.game}`);
    setText("playTitle", play.play);
    setText("playOdds", play.odds);
    setText(
      "playUnits",
      Number(play.units || 0).toFixed(2)
    );
    setText(
      "playRating",
      "★".repeat(Number(play.rating || 0))
    );
    setText("playHandicapper", play.handicapper);

    const analysis =
      document.getElementById("playAnalysis");

    analysis.innerHTML = "";

    String(
      play.analysis || "Analysis has not been posted."
    )
      .split(/\n\s*\n/)
      .map(text => text.trim())
      .filter(Boolean)
  ...
  deepDiveLink.href =
    ...
    status.textContent =
      error.message || "Unable to load play.";
  }
}
...
function setLogo(id, teamId, team) {
  ...
  img.src =
    `${PLAY_LOGO_BASE}/${Number(teamId)}.svg`;
